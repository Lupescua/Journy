@extends('layout')
@section('title')
Journy
@endsection


@section('content')
    <img src="https://static.pexels.com/photos/414171/pexels-photo-414171.jpeg" width="100%" height="80%" style="position : absolute" alt="Rick and Morty" >
    <form class="form-inline" style="position:relative; " method="post">
        <input class="form-control  font-weight-bold" style="background: #fff;margin:auto;"type="search" placeholder="Search" aria-label="Search" style="background-color:rgba(0,0,0,0);">
        <i class="material-icons color-black-50 search-button-icon">search</i>
    </form>
@endsection